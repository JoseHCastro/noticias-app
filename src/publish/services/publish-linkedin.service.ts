...
@Injectable()
export class PublishLinkedinService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (!this.linkedinToken) {
      console.warn('LINKEDIN_TOKEN no configurado en .env');
    }
  }
  ...
  async publish(caption: string, imageUrl: string): Promise<PublishResult> {
    try {
      ...
      console.log('Paso 1/4: Obteniendo información del usuario...');
      ...
      console.log('Paso 2/4: Registrando imagen...');
      ...
      console.log('Paso 3/4: Subiendo imagen...');
      ...
      console.log('Paso 4/4: Creando post...');
      ...
      console.error('Error al publicar en LinkedIn:', error.message);
      return {
        success: false,
        platform: 'linkedin',
        error: error.message,
   ...
   * Retorna el Person URN necesario para crear posts
   */
  private async getUserInfo(): Promise<string | null> {
    try {
      ...
      console.log('Usuario obtenido:', personUrn);
      return personUrn;
    } catch (error) {
  ...
  /**
   * PASO 2: Registrar la imagen para subir
   * Endpoint: POST /assets?action=registerUpload
   * Retorna el uploadUrl para subir la imagen y el asset URN para referenciarla
   */
  private async registerImageUpload(
    ...
    try {
      ...
      console.log('Imagen registrada:', asset);
      return { uploadUrl, asset };
    } catch (error) {
      console.error('Error en registerImageUpload:', error.message);
      return null;
    }
  }
   ...
   * Primero descarga la imagen desde imageUrl, luego la sube a LinkedIn
   * Endpoint: PUT uploadUrl (obtenido en el paso 2)
   */
  private async uploadImage(imageUrl: string, uploadUrl: string): Promise<boolean> {
    try {
      ...
      if (!uploadResponse.ok) {
        console.error('Error al subir imagen a LinkedIn');
        return false;
      }
